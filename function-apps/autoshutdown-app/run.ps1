...
function GetShutDownTag {
    $InstanceTags = $VM.tags.mw_autoshutdown
    return $InstanceTags
}
...
function SetShutDownTag {
    ...
    $Tag = @{mw_autoshutdown = $ShutDownTime}
...
function SetLastAutoshutdownEventTag {
    ...
    $Tag = @{mw_last_autoshutdown_event = $LastShutDownTime}
    Update-AzTag -Tag $Tag -ResourceId $VM.Id -Operation Merge | Out-Null
}
...
function getDevTestLabsSchedule {
    ...
    If yes, we extract the schedule time and set the mw_autoshutdown tag accordingly. 
    #>    
    try{
    ...
    if ($AutoShutdownSchedule.Properties.status -eq "Enabled"){
        ...
        $AutoShutDownDateTime = [System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId($AutoShutDownDateTime, $TimeZoneId, "GMT Standard Time");
...
function main {
    ...
    $Tag = $VM.tags.user_managed_shutdown

    if ($AutoShutDownDateTime){
        <# 
        If an Azure DevTest Labs auto-shutdown schedule if active for the VM, we check and set the user_managed_shutdown tag to true
        ...
        if (!$Tag -or ($Tag -eq "False")){
            ...
            #>
            $TagValue = @{user_managed_shutdown = "True"}
    ...
    # If in-built auto-shutdown is not active and the user_managed_shutdown tag is 'True' or non-existent, set it to true
    if ($Tag -ne "False"){
        $TagValue = @{user_managed_shutdown = "False"}
        Update-AzTag -Tag $TagValue -ResourceId $VM.Id -Operation Merge | Out-Null
    }
    
    # Once in-built auto-shutdown is handled, retrieve mw_autoshutdown tag
    $ShutDownTag = GetShutDownTag

    # Initialize the mw_autoshutdown tag according to user's choice for shut down
    if (!$ShutDownTag){
    ...
    # This is to check the status of the mw_autoshutdown tag in subsequent runs
